...
def train(character: Character, activity_key: str) -> list[str]:
    ...
    for attribute, base_gain in activity.attribute_gains.items():
        ...
        actual_gain = base_gain * fatigue_mod * cap_mod * character.talent_multiplier(activity.name) * character.affinity_multiplier(activity.name)
    ...
    for skill_name, base_gain in activity.skill_gains.items():
        gain = base_gain * fatigue_mod * character.talent_multiplier(skill_name) * character.affinity_multiplier(skill_name)
    ...
    if activity_key == "sword" and character.skill("Swordsmanship").proficiency >= 5 and any(t.name == "Sword Genius" and not t.discovered for t in character.talents):
        for talent in character.talents:
            if talent.name == "Sword Genius":
                talent.discovered = True
                messages.append("After repeated blade work, something clicks. Talent Discovered: Sword Genius")
                break
    return messages
